...
typedef bool(__stdcall* IsCallbackUsedProc)(LPCWSTR MethodName);
CallbackProc HookCallback = nullptr;
LogCallbackProc LogCallback = nullptr;
IsCallbackUsedProc IsCallbackUsed = nullptr;
...
extern "C" {
	...
	__declspec(dllexport) void __stdcall SetCallbacks(CallbackProc HookCallback, LogCallbackProc LogCallback, IsCallbackUsedProc IsCallbackUsed);
...
struct VTableHook
{
	...
	HRESULT Patch()
	{
		...
		*PointerInVTable = Override;
		if (!VirtualProtect(PointerInVTable, sizeof(Delegate), OldProtect, &OldProtect)) {
			return MAKE_HRESULT(SEVERITY_ERROR, FACILITY_WIN32, GetLastError());
		}
		return S_OK;
	}
...
struct DetourHook
{
	...
	{
		DWORD OldProtect = 0;
		if (!VirtualProtect(reinterpret_cast<void*>(MemoryOriginalProc), 14, PAGE_EXECUTE_READWRITE, &OldProtect)) {
			return MAKE_HRESULT(SEVERITY_ERROR, FACILITY_WIN32, GetLastError());
		}
		...
		*reinterpret_cast<intptr_t*>(DetourJump64 + 6) = Override;
		...
		if (!VirtualProtect(reinterpret_cast<void*>(MemoryOriginalProc), 14, OldProtect, &OldProtect)) {
			return MAKE_HRESULT(SEVERITY_ERROR, FACILITY_WIN32, GetLastError());
		}
		return S_OK;
	}
...
void __stdcall SetCallbacks(CallbackProc HookCallback, LogCallbackProc LogCallback, IsCallbackUsedProc IsCallbackUsed)
{
	::HookCallback = HookCallback;
	::LogCallback = LogCallback;
	::IsCallbackUsed = IsCallbackUsed;
}

#define InitHook(Name, PointerInVTable, Override) if (IsCallbackUsed(L#Name)) \
{ \
